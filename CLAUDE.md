...
- **DJI-FlightHub-2 Business-API**: Laut offizieller Apifox-Doku (2026-07-22 verifiziert, s. `docs/dji-flighthub2-api.md`) ist die Basis-URL für Cloud direkt `https://fh.dji.com` + `/openapi/v0.1/...` — so nutzt es auch v1s Code (`FireFlight/backend/modules/flighthub/service.py`), keine Hinweise auf ein separates `djigate.com`-Gateway gefunden. Die frühere Notiz hier ("fh.dji.com liefert nur HTML, echte Basis-URL ist ein Gateway wie `es-flight-api-us.djigate.com`") ließ sich weder im v1-Code noch in der aktuellen Doku bestätigen — falls das bei einem früheren echten Verbindungsversuch tatsächlich auftrat, vor der nächsten echten Anbindung erneut per Browser-DevTools (F12 → Netzwerk) gegenprüfen. Bestätigt: Jeder Request braucht `X-Request-Id`-Header (UUID) sowie `X-Language`. Ob leere Listen `"list": null` statt `[]` liefern, ist in der aktuellen Doku nicht explizit dokumentiert — beim Parsen defensiv bleiben (`(x.get("data") or {}).get("list") or []`).

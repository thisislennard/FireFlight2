...
- **DJI FlightHub 2 als zentrale Datenquelle, nicht Nebenmodul** — Datenfluss `DJI FlightHub 2 → DJI-Integrationsschicht (Client/Auth/Sync-Service/Mapper/Validierung/Fehlerbehandlung/Sync-Protokoll) → interne Datenmodelle → Services/Fachmodule → Dashboards/UI`. Fachmodule dürfen **nie** direkt mit der DJI-API sprechen. In dieser Phase: Mock-/Demo-Connector statt echter Anbindung (keine bestätigten Zugangsdaten/Endpunkte für FireFlight2 vorhanden — v1s bereits recherchierte Endpunkt-Referenz `FireFlight/docs/FLIGHTHUB2_API.md` kann als Ausgangspunkt dienen, sobald es an die echte Anbindung geht), Admin-Statusseite mit simulierter Synchronisierung. Herkunft von Daten wird generisch über `external_references` (Quelle, externe ID, Sync-Zeitpunkte, Fehler) modelliert, damit Fachmodelle nicht mit DJI-Feldern überladen werden

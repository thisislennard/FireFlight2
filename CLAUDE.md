...
## Umsetzungsstand
Ausbaustufe 1 aus `docs/spec-struktur.md` ist **implementiert und lokal live verifiziert** (venv + lokales PostgreSQL, da auf dieser Maschine kein Docker installiert ist): Datenmodell, Auth, Rollen/Berechtigungen, Dashboards, Modul-Registry, Administration, CLI-Init, Templates/Design. Ausbaustufe 2 (Fachmodule + PWA/Push aus `fireflight2-konzept-struktur.md`) ist **inhaltlich vollständig umgesetzt** — alle 15 Phasen des Restrukturierungsplans sind fertig und live verifiziert: Phase 1 (Modul-Registry-Bootstrap), Phase 2 (Rollen ohne Dashboard), Phase 3 (PIN-Login-Migration, Passwort komplett ersetzt), Phase 4 (Notifications-Kern, Web-Push), Phase 5 (RC-Hardware-Feasibility-Spike, reduzierter Umfang), Phase 6 (Drohneneinheiten), Phase 7 (Nutzerprofil-Erweiterung inkl. scharf geschaltetem RC-Qualifikationsfilter), Phase 8 (generische Wizard-Engine), Phase 9 (Einsatz/Übung + Flugbuch mit Karte), Phase 10 (Tickets + Wartungsintervalle, Gerätewart als erste Rolle ohne Dashboard), Phase 11 (Teilumfang: Zwei-Schritt-RC-Login), Phase 12 (RC-Wizard-Inhalte, voller Umfang inkl. Startanfrage-Genehmigung + Push), Phase 13 (fachliche Dashboard-Module: Flugbuch-Karte, Ticket-Meldeformular), Phase 14 (DWD-Wetter + OpenSky als "schlanke Direktanbindung" ohne eigene Integrationsschicht) und Phase 15 (Tests/Dokumentation: README + `docs/architecture.md` auf den Gesamtstand gebracht). Als Nachtrag danach zusätzlich die **Büro-PWA** umgesetzt (Nutzerwunsch 2026-07-24, keinem der 15 Phasen zugeordnet gewesen): Desktop-Oberfläche jetzt wie die RC-Seite installierbar (`app/static/manifest.webmanifest`, root-skopierter Service Worker über `/sw.js`, `app/static/js/pwa.js`), 236/236 Tests. **Verbleibend, nur vom Nutzer selbst durchführbar:** die Hardware-Verifikation auf der echten DJI RC Plus (Phase 4/5/12 — nur Code + automatisierte Tests + `curl`-Rundlauf gegen den Dev-Server sind bisher verifiziert), ein echter Docker-Compose-Lauf (nie getestet, kein Docker auf dieser Maschine) und die Büro-PWA-Installation in einem echten Browser. Die zuvor implementierte DJI-FlightHub-2-Integration wurde am 2026-07-23 auf Nutzerwunsch **komplett wieder entfernt** (kein Fachmodul/keine Integration soll den Kern ablenken, bevor dieser fertig steht) — Details im Verlauf unten. Details, offene Punkte und nächste Schritte: `docs/roadmap.md`.
...
### 2026-07-24 (Fortsetzung) — Nachtrag: Büro-PWA (auf Nutzerwunsch nach Phase 15)
Die zuvor als offen dokumentierte Frage ("ob/wann eine Büro-PWA nachgezogen wird") wurde vom Nutzer
direkt nach Abschluss von Phase 15 beantwortet: ja, gewünscht. Analog zum bestehenden RC-PWA-Muster
umgesetzt: `app/static/manifest.webmanifest` (Scope `/`), root-skopierter Service Worker über eine
neue Route `/sw.js` (`app/__init__.py: service_worker()`, exakt wie `app/rc/routes.py:
service_worker()` für `/rc/`). Anders als beim RC-Zugang **kein Content-Fork** nötig -- `/sw.js`
liefert dieselbe Datei wie das bestehende `/static/js/sw.js` aus Phase 4 aus, dessen Kommentar
diese spätere Nutzung bereits vorwegnahm. Neues `app/static/js/pwa.js` registriert den Worker
proaktiv auf jeder Seite (Installierbarkeits-Kriterien verlangen eine aktive Registrierung vor dem
Installieren-Dialog), `notifications.js`s `DEFAULT_SW_URL` auf `/sw.js` konsolidiert. Nebenfund:
ein Zwischenstand zwischen zwei Edit-Schritten (Funktionsaufruf vor Funktionsdefinition) wurde vom
Werkzeug-Auto-Reloader aufgegriffen und crashte den laufenden Dev-Server kurzzeitig -- reines
Timing-Artefakt der Live-Bearbeitung, kein Anwendungsfehler, nach Abschluss neu gestartet. 4 neue
Tests, Suite 236/236 grün. Live verifiziert: Manifest/`sw.js` korrekt ausgeliefert (u. a. korrekter
`Content-Type`). Der eigentliche Installieren-Dialog in einem echten Browser bzw. hinter echtem
HTTPS ist wie bei der RC-Hardware nur vom Nutzer selbst prüfbar. Details:
`docs/roadmap.md` Abschnitt „Status: Ausbaustufe 2" → „Nachtrag — Büro-PWA".

### 2026-07-24 (Fortsetzung) — Phase 15: Tests und Dokumentation (15-Phasen-Plan damit inhaltlich abgeschlossen)
Letzte Phase des Restrukturierungsplans, im Repo nur mit knappem Titel benannt (voller Plantext
liegt außerhalb des Repos und war nicht mehr auffindbar) -- pragmatisch als "README/Architekturdoku
auf Gesamtstand bringen + Testabdeckung prüfen" ausgelegt. `README.md` war noch komplett auf
Ausbaustufe 1 stehen geblieben (u. a. die falsche Aussage "fachliche Module noch nicht enthalten")
und wurde vollständig überarbeitet: neue Abschnitte für PIN-Login, RC-PWA, Wizard-System, Web-Push,
alle vier Fachmodule, Nutzerprofile/Einheiten, Testdaten; Projektstruktur/Env-Variablen/Tests-Zahlen
aktualisiert. Dabei Nebenfund: README verlangte Python 3.12, lokales `.venv` läuft aber unter
3.10.11 -- als bekannte, ungeprüfte Diskrepanz dokumentiert statt stillschweigend eine Zahl zu
ändern. `docs/architecture.md` um einen neuen Abschnitt "Ausbaustufe 2 — Entscheidungen" ergänzt,
der die über die Phasen verstreuten, weiterhin gültigen Architekturmuster bündelt (Berechtigungs-
prüfung in Fachmodul-Widgets, RC-Doppel-Session, Wizard-Engine-Neutralität, Magic-Byte-Uploads,
Phase-14-Cache-Strategie, punktuelle CSP-Lockerungen) -- die Phase-für-Phase-Historie bleibt bewusst
nur in `docs/roadmap.md`. Testabdeckung geprüft: keine echte Lücke gefunden (Administrationsrouten
sind über die jeweiligen Fachbereichs-Testdateien mitabgedeckt), daher keine neuen Tests. Suite
weiterhin 232/232 grün. Damit ist der 15-Phasen-Plan **inhaltlich vollständig umgesetzt** --
verbleibend sind nur zwei Punkte, die ausschließlich der Nutzer selbst durchführen kann: die
Hardware-Verifikation auf der echten DJI RC Plus und ein echter Docker-Compose-Lauf. Details:
`docs/roadmap.md` Abschnitt „Status: Ausbaustufe 2" → Phase 15.
